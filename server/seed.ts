import { db } from "./db";
import { customers, contacts, locations, serviceTypes, appointments, serviceRecords, productApplications, invoices, communications } from "@shared/schema";
...
export async function seedDatabase() {
  ...
  const [l1, l2, l3, l4, l5, l6] = await db.insert(locations).values([
    { customerId: c1.id, name: "Home", address: "1234 Oak Street", city: "Springfield", state: "IL", zip: "62701", isPrimary: true, propertyType: "residential", squareFootage: 2400, notes: "Single story ranch with large backyard" },
    ...
    { customerId: c3.id, name: "Home", address: "321 Pine Lane", city: "Springfield", state: "IL", zip: "62703", isPrimary: true, propertyType: "residential", squareFootage: 3100, notes: "Two story colonial with basement. History of termite activity." },
  ...
  await db.insert(invoices).values([
    { customerId: c1.id, serviceRecordId: sr1.id, invoiceNumber: "INV-001", amount: "125.00", tax: "10.00", totalAmount: "135.00", status: "paid", dueDate: twoDaysAgo, paidDate: yesterday, notes: "Monthly pest prevention" },
    { customerId: c2.id, serviceRecordId: sr2.id, invoiceNumber: "INV-002", amount: "250.00", tax: "20.00", totalAmount: "270.00", status: "pending", dueDate: nextWeek, notes: "Commercial kitchen service - Downtown" },
    { customerId: c4.id, invoiceNumber: "INV-003", amount: "375.00", tax: "30.00", totalAmount: "405.00", status: "paid", dueDate: lastMonth, paidDate: lastMonth, notes: "Quarterly service - both buildings" },
    { customerId: c3.id, invoiceNumber: "INV-004", amount: "200.00", tax: "16.00", totalAmount: "216.00", status: "pending", dueDate: twoWeeks, notes: "Annual termite inspection" },
  ]);

  await db.insert(communications).values([
    { customerId: c1.id, type: "email", direction: "outbound", subject: "Service Confirmation", body: "Hi James, this is to confirm your monthly pest prevention service has been completed. Your next service is scheduled for next month. Please don't hesitate to reach out if you notice any pest activity.", sentAt: twoDaysAgo, status: "sent" },
    { customerId: c2.id, type: "phone", direction: "inbound", subject: "Scheduling Call", body: "Sarah called to confirm next month's service schedule for both locations. Requested Tuesday mornings for Downtown and Thursday afternoons for Westside.", sentAt: yesterday, status: "logged" },
    { customerId: c3.id, type: "email", direction: "outbound", subject: "Upcoming Termite Inspection Reminder", body: "Hi Mike, just a reminder that your annual termite inspection is scheduled for tomorrow. Our technician Jake will arrive between 9-10 AM. Please ensure access to the crawl space and attic areas.", sentAt: now, status: "sent" },
    ...
    { customerId: c4.id, type: "sms", direction: "outbound", subject: "Service Reminder", body: "Hi Linda, reminder: quarterly pest service for Sunset Apartments is scheduled in 3 days. Please ensure common areas are accessible.", sentAt: now, status: "sent" },
  ]);

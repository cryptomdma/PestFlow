...
| Invoice document: Bill To from the primary location / billing profile; a Service Location block (owner, 2026-09-21) | **DEFECT** / ABSENT | `getInvoiceDocumentContext` (`storage.ts:7767-7777`) falls back to the **service** location's live address when no profile address was snapshotted, which is every invoice on the dev DB (two profile rows exist, neither with an address; 45 of 64 invoices have no snapshot); the renderer prints Remit To and Bill To only (`invoice-pdf.ts:44-56`, `invoice-html.ts:66-79`). C2.1c |
...
reschedule). The rest in phase order. 11c and 11d are inserted ahead of 12 (owner, 2026-09-21)

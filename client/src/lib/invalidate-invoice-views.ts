 ...
 * "initial-charge-invoice"]. A prefix match on the first element would miss
 * most of these, so match on the string itself. Money moving changes every
 * one of them (PLAN_BILLING_V1_1.md D5), which is why there is one
 * invalidation rather than a list per call site.
 */

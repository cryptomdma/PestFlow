> **HISTORICAL — SUPERSEDED. Not current guidance. Do not cite going forward.**
>
> This is the original Phase 1 billing plan, restored 2026-09-10 for one reason only: roughly two
> dozen `PLAN_BILLING_V1.md §x.x` citations survive in `shared/schema.ts`, `server/storage.ts`,
> `server/jobs/billing-run.ts` and elsewhere, and with the file gone they pointed at section numbers
> nobody could open. It is retained so those resolve.
>
> It was removed from the repo deliberately, to keep the active doc set small — every session starts a
> fresh agent from `CLAUDE.md`'s reading list, and a stale plan sitting alongside the current one is
> exactly how a session picks up the wrong instructions. It is therefore **not** on that list and must
> not be added to it. Read it only to resolve an existing citation.
>
> **`PLAN_BILLING_V1_1.md` governs.** Known contradictions, so nothing here is mistaken for live
> policy:
> - "One invoice per service ticket" is superseded by **D1** — invoices anchor to the appointment,
...
>   earning commission. It also names no field recording **who sold** an agreement, which
>   `COMMISSION_ON_NEW_AGREEMENT` needs to resolve a payee. See the compensation-and-attribution
>   entry in `CURRENT_FOCUS.md` for the three extensions.
>
> Everything below this line is the untouched original.
